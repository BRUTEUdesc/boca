...
require_once 'polygon_utils.php';
...
function DBGetProblemData($contestnumber, $problemnumber, $c = null)
{
    ...
    $r = DBExec(
        $c,
        "select p.problemname as problemname, p.problemfullname as fullname, p.problembasefilename " .
...
function DBGetFullProblemData($contestnumber, $freeproblems = false)
{
    ...
    $r = DBExec(
        $c,
        "select p.problemnumber as number, p.problemname as name, p.problemfullname as fullname, " .
        ...
        if ($freeproblems && $a[$i]['fake'] != 't') {
            if (is_readable($ptmp . ".name")) {
                ...
                if (!$failed) {
                    ...
                    if (!$failed) {
                        ...
                        if (!$failed) {
                            DBExec(
                                $c,
                                "update problemtable set problemfullname='$fullname', problembasefilename='$basename' where problemnumber=$nn and contestnumber=$contestnumber",
                ...
                if ($failed) {
                    ...
                    DBExec(
                        $c,
                        "update problemtable set problemfullname='', problembasefilename='' where problemnumber=$nn and contestnumber=$contestnumber",
...
function DBNewProblem($contestnumber, $param, $c = null)
{
    ...
    if (isset($param['problemnumber']) && !isset($param['number'])) {
        $param['number'] = $param['problemnumber'];
    }
    if (isset($param['problemname']) && !isset($param['name'])) {
        $param['name'] = $param['problemname'];
    }
    if (isset($param['problemfullname']) && !isset($param['fullname'])) {
        $param['fullname'] = $param['problemfullname'];
    }
    if (isset($param['problembasefilename']) && !isset($param['basename'])) {
        $param['basename'] = $param['problembasefilename'];
    }
    if (isset($param['problemcolorname']) && !isset($param['colorname'])) {
        $param['colorname'] = $param['problemcolorname'];
    }
    if (isset($param['problemcolor']) && !isset($param['color'])) {
        $param['color'] = $param['problemcolor'];
    }
    if (isset($param['problemautojudge']) && !isset($param['autojudge'])) {
        $param['autojudge'] = $param['problemautojudge'];
    }
    if (isset($param['probleminputfile']) && !isset($param['inputfilepath'])) {
        $param['inputfilepath'] = $param['probleminputfile'];
    }
    if (isset($param['probleminputfilename']) && !isset($param['inputfilename'])) {
        $param['inputfilename'] = $param['probleminputfilename'];
    }
    if (isset($param['basename'])) {
    ...
    if ($inputfilepath != "" && is_polygon_package($inputfilepath)) {
        $converted = convert_polygon_to_boca($inputfilepath, $name);
        if ($converted === false) {
            MSGError("Error converting polygon package to BOCA package.");
            ForceLoad("problem.php");
            exit;
        }
        list($inputfilepath, $inputfilename) = $converted;
    }

    // Debug output for autojudge parameter
    // echo "<script>console.log('autojudge: " . $autojudge . "');</script>";
...
function DBGetProblems($contest, $showanyway = false)
{
    ...
    if (($c = DBContestInfo($contest)) == null) {
        return array();
    }
    if (time() < $c['conteststartdate'] && !$showanyway) {
        return array();
    }
