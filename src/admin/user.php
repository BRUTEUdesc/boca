...
if (isset($_POST["username"]) && isset($_POST["userfullname"]) && isset($_POST["userdesc"]) && isset($_POST["userip"]) &&
    ...
    $param['changepass'] = 't';
    if (isset($_POST['changepass']) && $_POST['changepass'] != 't') {
        $param['changepass'] = 'f';
    }
    /*
    ...
    */
    ...
    ForceLoad("user.php");
} elseif (isset($_FILES["importfile"]) && isset($_POST["Submit"]) && $_FILES["importfile"]["name"] != "") {
    if ($_POST["confirmation"] == "confirm") {
        ...
        } elseif (strtolower(substr($name, -4)) == ".tab") {
            $oklines = 0;
            ...
            for ($i++; $i < count($ar) && $ar[$i][0] != "["; $i++) {
                ...
                if (strpos($x, "user") !== false && strpos($x, "user") == 0) {
                    ...
                    while (strpos($x, "user") !== false && strpos($x, "user") == 0) {
                        $tmp = explode("=", $x, 2);
                        switch (trim($tmp[0])) {
                            case "usersitenumber":    $param['site'] = trim($tmp[1]);
                                break;
                            case "username":          $param['username'] = trim($tmp[1]);
                                break;
                            case "usericpcid":        $param['usericpcid'] = trim($tmp[1]);
                                break;
                            case "usernumber":        $param['usernumber'] = trim($tmp[1]);
                                break;
                            case "userfullname":      $param['userfull'] = trim($tmp[1]);
                                break;
                            case "userdesc":          $param['userdesc'] = trim($tmp[1]);
                                break;
                            case "usertype":          $param['type'] = trim($tmp[1]);
                                break;
                            case "userenabled":       $param['enabled'] = trim($tmp[1]);
                                break;
                            case "usermultilogin":    $param['multilogin'] = trim($tmp[1]);
                                break;
                            case "userpassword":      $param['pass'] = myhash(trim($tmp[1]));
                                break;
                            case "userchangepassword": $param['changepass'] = trim($tmp[1]);
                                break;
                            case "userip":            $param['permitip'] = trim($tmp[1]);
                                break;
                        }
                        $i++;
                        if ($i >= count($ar)) {
   ...
   <center>
    <table border="0">
      <tr> 
        ...
        <td width="65%">
	  <input type="text" name="usersitenumber" <?php if (!$main) {
	      echo "readonly ";
	  } echo "value=\"" . $usite . "\""; ?> size="20" maxlength="20" />
        </td>
      </tr>
      <tr> 
        <td width="35%" align=right>User Number:</td>
        <td width="65%">
	  <input type="text" name="usernumber" value="<?php if (isset($u)) {
	      echo $u["usernumber"];
      ...
      <tr> 
        ...
        <td width="65%">
	  <input type="text" name="username" value="<?php if (isset($u)) {
	      echo $u["username"];
	  } ?>" size="20" maxlength="20" />
        </td>
      </tr>
      <tr> 
        ...
        <td width="65%">
	  <input type="text" name="usericpcid" value="<?php if (isset($u)) {
	      echo $u["usericpcid"];
      ...
      <tr> 
		...
		<option <?php if (!isset($u) || $u["usertype"] == "team") {
		    echo "selected";
		} ?> value="team">Team</option>
		<option <?php if (isset($u)) {
		    if ($u["usertype"] == "judge") {
		        echo "selected";
		    }
		} ?> value="judge">Judge</option>
		<option <?php if (isset($u)) {
		    if ($u["usertype"] == "admin") {
		        echo "selected";
		    }
      ...
      <tr> 
		...
		<option <?php if (!isset($u) || $u["userenabled"] != "f") {
		    echo "selected";
		} ?> value="t">Yes</option>
		<option <?php if (isset($u) && $u["userenabled"] == "f") {
		    echo "selected";
      ...
      <tr> 
		...
		<option <?php if (isset($u) && $u["usermultilogin"] == "t") {
		    echo "selected";
		} ?> value="t">Yes</option>
		<option <?php if (!isset($u) || $u["usermultilogin"] != "t") {
		    echo "selected";
      ...
      <tr> 
        ...
        <td width="65%">
	  <input type="text" name="userfullname" value="<?php if (isset($u)) {
	      echo $u["userfullname"];
	  ...
	  <input type="text" name="userdesc" value="<?php if (isset($u)) {
	      ...
	      echo $u["userdesc"];
	  } ?>" size="50" maxlength="300" />
        </td>
      </tr>
      <tr> 
        <td width="35%" align=right>User IP:</td>
        <td width="65%">
	  <input type="text" name="userip" value="<?php if (isset($u)) {
	      echo $u["userpermitip"];
	  } ?>" size="20" maxlength="20" />
        </td>
      </tr>
      ...
      <tr> 
		...
		<option <?php if (isset($u) && $u["changepassword"]) {
		    echo "selected";
		} ?> value="t">Yes</option>
		<option <?php if (!isset($u) || !$u["changepassword"]) {
		    echo "selected";
