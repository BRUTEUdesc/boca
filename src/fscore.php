...
function ScoreCMP($a, $b)
{
    ...
    if ($a["totalcount"] > $b["totalcount"]) {
        return 1;
    } elseif ($a["totalcount"] < $b["totalcount"]) {
        return -1;
    } else {
...
function DBScore($contest, $verifylastmile, $hor = -1, $globalsite = '0')
{
    ...
    $r = DBExec(
        $c,
        "select sitenumber as number from sitetable where contestnumber=$contest and siteactive='t'",
        "DBScore(get site)"
    );
    $n = DBnlines($r);
    if ($n == 0) {
                    ...
                    if ($fine) {
                        ...
                        if (false) {
                            ...
                            while (list($ee, $cc) = each($arrori)) {
                                for ($pi = 0; $pi < 11; $pi++) {
                                    unset($arr[$ee]['problem'][$pi + 1]);
                                }
                                reset($arrori[$ee]["problem"]);
                                while (list($e2, $c2) = each($arrori[$ee]["problem"])) {
                                    for ($pi = 0; $pi < 11; $pi++) {
                                        if (isset($arrori[$ee]['problem'][$e2]['name']) && trim($arrori[$ee]['problem'][$e2]['name']) == $pname[$pi]) {
                                            break;
                                        }
                                    }
                                    if ($pi < 11) {
