...
-- (esta tabela contem uma linha para cada reinicio de um site do contest)
    ...
    $r = DBExec(
        $c,
        "CREATE UNIQUE INDEX \"sitetime_index\" ON \"sitetimetable\" USING btree ".
...
-- (esta tabela contem uma linha para cada usuario, seja ele administrador, juiz ou time. )
    ...
    $r = DBExec(
        $c,
        "CREATE UNIQUE INDEX \"user_index\" ON \"usertable\" USING btree ".
         "(\"contestnumber\" int4_ops, \"usersitenumber\" int4_ops, \"usernumber\" int4_ops)",
        "DBCreateUserTable(create user_index)"
    );
    $r = DBExec(
        $c,
        "CREATE UNIQUE INDEX \"user_index2\" ON \"usertable\" USING btree ".
    ...
    if (isset($inst[1])) {
        ...
        if (isset($inst[2])) {
            ...
            if (isset($inst[1])) {
                ...
                if (isset($inst2[1])) {
                    $a['usersitename'] = strtoupper(trim($inst2[1]));
                }
            }
        } else {
...
function DBUpdateContest($param, $c = null)
{
    ...
    if (isset($param['contestname']) && !isset($param['name'])) {
        $param['name'] = $param['contestname'];
    }
    if (isset($param['conteststartdate']) && !isset($param['startdate'])) {
        $param['startdate'] = $param['conteststartdate'];
    }
    if (isset($param['contestduration']) && !isset($param['duration'])) {
        $param['duration'] = $param['contestduration'];
    }
    if (isset($param['contestlastmileanswer']) && !isset($param['lastmileanswer'])) {
        $param['lastmileanswer'] = $param['contestlastmileanswer'];
    }
    if (isset($param['contestlastmilescore']) && !isset($param['lastmilescore'])) {
        $param['lastmilescore'] = $param['contestlastmilescore'];
    }
    if (isset($param['contestpenalty']) && !isset($param['penalty'])) {
        $param['penalty'] = $param['contestpenalty'];
    }
    if (isset($param['contestmaxfilesize']) && !isset($param['maxfilesize'])) {
        $param['maxfilesize'] = $param['contestmaxfilesize'];
    }
    //  if(isset($param['contestactive']) && !isset($param['active'])) $param['active']=$param['contestactive'];
    if (isset($param['contestmainsite']) && !isset($param['mainsite'])) {
        $param['mainsite'] = $param['contestmainsite'];
    }
    if (isset($param['contestkeys']) && !isset($param['keys'])) {
        $param['keys'] = $param['contestkeys'];
    }
    ...
    if ($active == 1) {
        ...
        DBExec(
            $c,
            "update contesttable set contestactive='t' where contestnumber=$number",
...
function DBRenewSiteTime($param, $c = null)
{
    ...
    for ($i = 0; isset($param[$i]); $i++) {
        ...
        if (isset($param[$i]['contestnumber']) && !isset($param[$i]['contest'])) {
            $param[$i]['contest'] = $param[$i]['contestnumber'];
        }
        if (isset($param[$i]['sitenumber']) && !isset($param[$i]['site'])) {
            $param[$i]['site'] = $param[$i]['sitenumber'];
        }
        foreach ($ac as $key) {
        ...
        if (!isset($param[$i]['updatetime'])) {
            $param[$i]['updatetime'] = $t;
        }
        if ($param[$i]['updatetime'] > $maxtime) {
            $maxtime = $param[$i]['updatetime'];
        }
        if (!isset($param[$i]['enddate'])) {
            $param[$i]['enddate'] = 0;
        }
    ...
    if ($cw) {
        DBExec($c, "commit work", "DBRenewSiteTime(commit)");
    }
    return $ret;
}
function DBNewContest($param = array(), $c = null)
{
    ...
    $a = DBGetRow(
        "select max(contestnumber) as contest from contesttable",
        0,
        $c,
...
function DBNewSite($contest, $c = null, $param = array())
{
    ...
    if (isset($param['siteduration']) && !isset($param['duration'])) {
        $param['duration'] = $param['siteduration'];
    }
...
function DBNewUser($param, $c = null)
{
    ...
    if (isset($param['usersitenumber']) && !isset($param['site'])) {
        $param['site'] = $param['usersitenumber'];
    }
    ...
    if (isset($param['userpassword']) && !isset($param['pass'])) {
        $param['pass'] = $param['userpassword'];
    }
    if (isset($param['userenabled']) && !isset($param['enabled'])) {
        $param['enabled'] = $param['userenabled'];
    }
    if (isset($param['usermultilogin']) && !isset($param['multilogin'])) {
        $param['multilogin'] = $param['usermultilogin'];
    }
    if (isset($param['userpermitip']) && !isset($param['permitip'])) {
        $param['permitip'] = $param['userpermitip'];
    }
    if (isset($param['userfullname']) && !isset($param['userfull'])) {
        $param['userfull'] = $param['userfullname'];
    }
    if (isset($param['usertype']) && !isset($param['type'])) {
        $param['type'] = $param['usertype'];
    }
    if (isset($param['userpermitip']) && !isset($param['permitip'])) {
        $param['permitip'] = $param['userpermitip'];
    }
    ...
    if ($pass != myhash("") && $type != "admin" && $changepass != "t" && substr($pass, 0, 1) != "!") {
        $pass = '!'.$pass;
    }
    $r = DBExec($c, "select * from usertable where username='$username' and usernumber!=$user and ".
        ...
        if ($a == null) {
            ...
            if ($updatetime > $a['updatetime']) {
                ...
                if ($userlastlogin != null) {
                    $sql .= "userlastlogin='$userlastlogin', ";
                }
