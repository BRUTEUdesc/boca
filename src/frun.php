...
CONSTRAINT \"lang_fk\" FOREIGN KEY (\"contestnumber\", \"runlangnumber\")
    ...
    $r = DBExec(
        $c,
        "CREATE UNIQUE INDEX \"run_index\" ON \"runtable\" USING btree ".
                "(\"contestnumber\" int4_ops, \"runsitenumber\" int4_ops, \"runnumber\" int4_ops)",
        "DBCreateRunTable(create run_index)"
    );
    $r = DBExec(
        $c,
        "CREATE INDEX \"run_index2\" ON \"runtable\" USING btree ".
...
function DBUpdateRunC($contest, $usersite, $usernumber, $runsite, $runnumber, $answer, $chief, $c = null)
{
    ...
    $a = DBGetRow(
        "select * from answertable where answernumber=$answer and contestnumber=$contest",
        0,
        $c,
    ...
    if ($temp["runjudge1"] == $usernumber && $temp["runjudgesite1"] == $usersite) {
        DBExec(
            $c,
            "update runtable set runanswer1=$answer, updatetime=".time()." " .
    ...
    if ($temp["runjudge2"] == $usernumber && $temp["runjudgesite2"] == $usersite) {
        DBExec(
            $c,
            "update runtable set runanswer2=$answer, updatetime=".time()." " .
               "where contestnumber=$contest and runnumber=$runnumber and runsitenumber=$runsite",
    ...
    if ($chief == 1 || ($outra != 0 && $outra == $answer && $temp["runstatus"] != "judged+") ||
       ($outra != 0 && $outra == $answer && $temp["runanswer"] == $answer)) {
        ...
        DBExec(
            $c,
            "update runtable set runstatus='judged', " .
...
function DBRunGiveUp($number, $site, $contest, $usernumber, $usersite)
{
    ...
    if ($temp["runjudge1"] == $usernumber && $temp["runjudgesite1"] == $usersite) {
        DBExec(
            $c,
            "update runtable set runjudge1=NULL, runjudgesite1=NULL, runanswer1=0 " .
    ...
    if ($temp["runjudge2"] == $usernumber && $temp["runjudgesite2"] == $usersite) {
        DBExec(
            $c,
            "update runtable set runjudge2=NULL, runjudgesite2=NULL, runanswer2=0 " .
               " where contestnumber=$contest and runnumber=$number and runsitenumber=$site",
            "DBRunGiveUp(update run judge1)"
        );
        $outra = $temp['runanswer1'];
    ...
    if ($temp["runstatus"] == "judged" || $temp["runstatus"] == "judged+") {
        ...
        if ($outra == 0 || $tx == "Chief") {
            DBExec(
                $c,
                "update runtable set runstatus='openrun', runanswer=0, runjudge=NULL, runjudgesite=NULL, ".
...
function DBRunDelete($number, $site, $contest, $user, $usersite, $cc = null)
{
    ...
    DBExec(
        $c,
        "update runtable set runstatus='deleted', runjudge=$user, runjudgesite=$usersite, updatetime=" .
           time()." where contestnumber=$contest and runnumber=$number and runsitenumber=$site",
        ...
        if (($a["judge1"] != $_SESSION["usertable"]["usernumber"] ||
           ...
           ($a["judge2"] != $_SESSION["usertable"]["usernumber"] ||
            ...
            if ($a["judge1"] == '' && $a['judgesite1'] == '') {
                DBExec(
                    $c,
                    "update runtable set runjudge1=" . $_SESSION["usertable"]["usernumber"] .
                       ",$upd updatetime=".time().", " .
                ...
                DBExec(
                    $c,
                    "update runtable set runjudge2=" . $_SESSION["usertable"]["usernumber"] .
                       ",$upd updatetime=".time().", " .
...
function DBUpdateRunAutojudging($contest, $site, $number, $ip, $answer, $stdout, $stderr, $retval = 0)
{
    ...
    DBExec(
        $c,
        "update runtable set autoenddate=$t, autoanswer=$answer, autostdout=$oid1, autostderr=$oid2, " .
           "updatetime=$t " .
           "where contestnumber=$contest and runnumber=$number and runsitenumber=$site",
     ...
     * if zero, do nothing
     */
    $autojudge_setting = ((int) $problem_data[0]['autojudge']);
...
function DBGiveUpRunAutojudging($contest, $site, $number, $ip = "", $ans = "", $fromadmin = false)
{
    ...
    if ($ip == "") {
        DBExec(
            $c,
            "update runtable set autoenddate=null, autoanswer=null, autostdout=null, autostderr=null, " .
        ...
        DBExec(
            $c,
            "update runtable set autoenddate=$t, autoanswer='$ans', autostdout=null, ".
...
function DBNewRun($param, $c = null, $allowinsert = true)
{
    ...
    if (isset($param['runsitenumber']) && !isset($param['site'])) {
        $param['site'] = $param['runsitenumber'];
    }
    ...
    if (isset($param['runfilename']) && !isset($param['filename'])) {
        $param['filename'] = $param['runfilename'];
    }
    if (isset($param['rundata']) && !isset($param['filepath'])) {
        $param['filepath'] = $param['rundata'];
    }
    ...
    if ($insert) {
        ...
        if ($runinc >= $runnumber) {
            while (true) {
                ...
                if (DBExecNonStop(
                    $c,
                    "INSERT INTO runtable (contestnumber, runsitenumber, runnumber, usernumber, rundate, " .
        ...
        if ($updatetime > $t) {
            ...
            if (isset($oldoid1) && is_numeric($oldoid1)) {
                DB_lo_unlink($c, $oldoid1);
            }
            if (isset($oldoid2) && is_numeric($oldoid2)) {
...
function DBUserRuns($contest, $site, $user)
{
    ...
    $r = DBExec(
        $c,
        "select distinct r.runnumber as number, r.rundatediff as timestamp, " .
...
function DBUserRunsYES($contest, $site, $user)
{
    ...
    $r = DBExec(
        $c,
        "select distinct p.problemcolorname as colorname, p.problemcolor as color, " .
...
function DBJudgedRuns($contest, $site, $user)
{
    ...
    $r = DBExec(
        $c,
        "select distinct r.runsitenumber as site, r.runnumber as number, r.rundatediff as timestamp, " .
                "p.problemname as problem, r.runstatus as status, l.langname as language, l.langextension as extension, " .
