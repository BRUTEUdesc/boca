    ...
    if ((isset($_POST["confirmation"]) && $_POST["confirmation"] == "confirm") || (isset($_POST["data"]) && isset($_POST["name"]))) {
        ...
        if (trim($linesubmission) == '1') {
            if (isset($_POST['comp']) && $_POST['comp'] != '') {
                $compv = substr(trim(sanitizeFilename($_POST['comp'])), 0, 150);
            } else {
        ...
        if (is_array($codes) && in_array($verify, $codes)) {
            ...
            MSGError("Same file already submitted for this problem and language");
            ForceLoad($runteam);
        }
        ...
        if (isset($_POST['pastcode']) && $_POST['pastcode'] != '') {
            ...
            if (isset($_POST["pasthash"]) && isset($_POST["pastval"])) {
                ...
                if (is_readable($_SESSION["locr"] . $ds . "private" . $ds . 'run-past.config')) {
                    ...
                    $pastsubmission = array_map(function ($element) {
                        $sp = explode(' ', $element, 4);
                        return trim($sp[2]);
                    }, $pcodes);
                    $key = -1;

                    for ($hh = 0; $hh < count($pastsubmission); $hh++) {
                        if (myhash($pastsubmission[$hh] . $pastcode . $compv . $pastval) == $pastvalhash) {
                            $key = $hh;
                            break;
                        }
                    }
            ...
            $tardes = array_map(function ($element) {
                $sp = explode(' ', $element, 4);
                if (count($sp) > 3) {
                    return 60 * trim($sp[3]);
                } return 0;
