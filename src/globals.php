...
function dirrec($dir, $user, $group, $dirPermissions, $filePermissions, $avoid = array())
{
    ...
    if (is_dir($dir)) {
        ...
        $ug = $u['gid'];
        if ($un != $user) {
            echo "user of $dir must be fixed!!\n";
        }
        if ($ug != $group) {
            echo "group of $dir must be fixed!!\n";
        }
        if (@chmod($dir, $dirPermissions) === false) {
            echo "cannot chmod $dir\n";
        }
        ...
        if (!is_link($dir)) {
            ...
            if (@chmod($dir, $filePermissions) === false) {
                echo "cannot chmod $dir\n";
            }
        }
    }
...
function fixbocadir($dir, $full = false)
{
    if (is_dir($dir)) {
        ...
        foreach ($d as $a) {
            cleardir($dir . $ds . 'private' . $ds . $a, true, true, false);
        }
...
function revertupdatebocafile($dirboca, $t)
{
    ...
    if (is_dir($dirboca)) {
        ...
        if (is_file($dirboca) && substr($dirboca, strlen($dirboca) - strlen('.' . $t . '.oldboca')) == '.' . $t . '.oldboca') {
            @chmod($dirboca, 0600);
            if (@copy($dirboca, substr($dirboca, 0, strlen($dirboca) - strlen('.' . $t . '.oldboca'))) === true) {
                $ok = 1;
            }
            @chmod($dirboca, 0000);
        }
...
function getIP()
{
    ...
    if (defined("dbcompat_1_4_1") && dbcompat_1_4_1 == true) {
        return $ip;
    }
    ...
    if (getenv("HTTP_X_CLIENTIP")) {
        ...
        $ip1a = strtok($ip1a, ",");
        if ($ip1a != $ip1 && $ip1a != getenv("REMOTE_ADDR")) {
            $ip .= ';' . $ip1a;
        }
    }
    if (getenv("HTTP_CLIENT_IP")) {
        ...
        $ip2 = strtok($ip2, ",");
        if ($ip2 != $ip1a && $ip1 != $ip2 && $ip2 != getenv("REMOTE_ADDR")) {
            $ip .= ';' . $ip2;
        }
    } else {
