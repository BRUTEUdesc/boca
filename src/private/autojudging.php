...
while (42) {
    ...
    if (is_readable($cache . $ds . $run["inputoid"] . "." . $run["inputname"])) {
        ...
        if (!is_readable($flocal)) {
            $flocal = '/root/icpc-latam-packages/' . trim($run["problemname"]) . ".ZIP";
        }
        if (!is_readable($flocal)) {
            $flocal = '';
        }
        ...
        if (($info = @parse_ini_file($dir . $ds . "problemdatalocal" . $ds . "description" . $ds . 'problem.info')) === false) {
            ...
            echo "Problem info obtained from local package file\n";
        }
