...
CONSTRAINT \"user_fk\" FOREIGN KEY (\"contestnumber\", \"sitenumber\", \"usernumber\")
    ...
    $r = DBExec(
        $c,
        "CREATE UNIQUE INDEX \"bkp_index\" ON \"bkptable\" USING btree ".
        "(\"contestnumber\" int4_ops, \"sitenumber\" int4_ops, \"bkpnumber\" int4_ops)",
        "DBCreateBkpTable(create bkp_index)"
    );
    $r = DBExec(
        $c,
        "CREATE INDEX \"bkp_index2\" ON \"bkptable\" USING btree ".
...
function DBBkpDelete($number, $site, $contest, $user, $adm = '')
{
    ...
    DBExec(
        $c,
        "update bkptable set bkpstatus='$str', updatetime=" .
...
function DBNewBkp($contest, $site, $user, $filename, $filepath, $size)
{
    ...
    DBExec(
        $c,
        "INSERT INTO bkptable (contestnumber, sitenumber, bkpnumber, usernumber, bkpdate, bkpfilename, bkpdata, bkpstatus, bkpsize) " .
