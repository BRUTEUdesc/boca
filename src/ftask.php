...
CONSTRAINT \"user_fk\" FOREIGN KEY (\"contestnumber\", \"sitenumber\", \"usernumber\")
    ...
    $r = DBExec(
        $c,
        "CREATE UNIQUE INDEX \"task_index\" ON \"tasktable\" USING btree ".
...
function DBUpdateTaskC($contest, $usersite, $usernumber, $tasksite, $tasknumber, $status, $chief)
{
    ...
    DBExec(
        $c,
        "update tasktable set taskstatus='$status', taskstaffnumber=$usernumber, taskstaffsite=$usersite, " .
...
function DBTaskGiveUp($number, $site, $contest, $usernumber, $usersite)
{
    ...
    DBExec(
        $c,
        "update tasktable set taskstatus='opentask', taskstaffnumber=NULL, taskstaffsite=NULL, ".
           "updatetime=" .time(). " ".
...
function DBTaskDelete($number, $site, $contest, $user, $usersite)
{
    ...
    DBExec(
        $c,
        "update tasktable set taskstatus='deleted', taskstaffnumber=$user, taskstaffsite=$usersite, ".
           "updatetime=" . time()." ".
           "where contestnumber=$contest and tasknumber=$number and sitenumber=$site",
...
function DBGetTaskToAnswerC($number, $site, $contest, $chief)
{
    ...
    if ($chief != 1) {
        DBExec(
            $c,
            "update tasktable set taskstaffnumber=" .
...
function DBNewTask($param, $c = null, $autotask = false)
{
    ...
    if (isset($param['taskfilename']) && !isset($param['filename'])) {
        $param['filename'] = $param['taskfilename'];
    }
    if (isset($param['taskdata']) && !isset($param['filepath'])) {
        $param['filepath'] = $param['taskdata'];
    }
    if (isset($param['taskstatus']) && !isset($param['status'])) {
        $param['status'] = $param['taskstatus'];
    }
    if (isset($param['taskdesc']) && !isset($param['desc'])) {
        $param['desc'] = $param['taskdesc'];
    }
    if (isset($param['tasksystem']) && !isset($param['sys'])) {
        $param['sys'] = $param['tasksystem'];
    }
    ...
    if ($insert) {
        ...
        if ($taskinc >= $tasknumber) {
            while (true) {
                ...
                if (DBExecNonStop(
                    $c,
                    "INSERT INTO tasktable (contestnumber, sitenumber, tasknumber, usernumber, taskdate, " .
...
function DBUserTasks($contest, $site, $user)
{
    ...
    $r = DBExec(
        $c,
        "select distinct t.sitenumber as site, t.tasknumber as number, ".
...
function DBCountOpenTasks($contest, $site, $user)
{
    ...
    $r = DBExec(
        $c,
        "select t.contestnumber, t.sitenumber, t.tasknumber ".
                             "from tasktable as t " .
                             "where t.contestnumber=$contest and t.sitenumber=$site and ".
