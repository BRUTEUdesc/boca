...
function DB_lo_read($contest, $id, $s = -1, $c = null)
{
    if (strcmp(phpversion(), '4.2.0') < 0) {
        if ($s < 0) {
            $str = '';
            while (($buf = pg_loread($id, 1000000)) != false) {
                $str .= $buf;
            }
        } else {
            $str = pg_loread($id, $s);
        ...
        if ($s < 0) {
            $str = '';
            while (($buf = pg_lo_read($id, 1000000)) != false) {
                $str .= $buf;
            }
        } else {
            $str = pg_lo_read($id, $s);
    ...
    if (($str2 = DB_unlock($contest, $str, $c)) === false) {
        return $str;
    }
    return $str2;
}
...
{
    if (($oid = DB_lo_create($conn)) === false) {
        return false;
    }
    if (($handle = DB_lo_open($conn, $oid, "w")) === false) {
        return false;
    }
    if (DB_lo_write($handle, $text) === false) {
        $oid = false;
    }
    DB_lo_close($handle);
    return $oid;
