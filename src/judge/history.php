...
if (count($clar) == 0) {
    ...
    echo "<br><b><font color=\"#b0b0a0\">* Shadowed clars and runs were judged by this judge</font></b>";
}
...
if (count($clar) > 0) {
    echo ((int) ($myclars * 1000 / count($clar))) / 10 . "%)<br>\n";
} else {
    echo "0%)<br>\n";
}
echo "My judged runs: " . $myruns . "/" . count($run) ." (";
if (count($run) > 0) {
    echo ((int) ($myruns * 1000 / count($run))) / 10 . "%)<br>\n";
} else {
    echo "0%)<br>\n";
}
echo "Accepted runs that I've judged: " . $myyes . "/" . $yes . " (";
if ($yes > 0) {
    echo ((int) ($myyes * 1000 / $yes)) / 10 ."%)<br>\n";
} else {
    echo "0%)<br>\n";
}
?>
