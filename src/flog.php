...
CREATE TABLE \"logtable\" (
    ...
    $r = DBExec(
        $c,
        "CREATE INDEX \"log_index\" ON \"logtable\" USING btree ".
         "(\"contestnumber\" int4_ops, \"sitenumber\" int4_ops, \"logdate\" int4_ops)",
        "DBCreateLogTable(create log_index)"
    );
    $r = DBExec(
        $c,
        "CREATE INDEX \"log_index2\" ON \"logtable\" USING btree ".
...
function DBNewLog($contest, $site, $user, $type, $ip, $data, $status)
{
    ...
    DBExecNoSQLLog(
        "insert into logtable (contestnumber, sitenumber, loguser, logdate, logtype, " .
...
function DBGetLogs($o, $contest, $site, $user, $type, $ip, $limit)
{
    ...
    switch ($o) {
        case "user": $order = "contestnumber, sitenumber, loguser, logdate desc";
            break;
        case "type": $order = "contestnumber, sitenumber, logtype, logdate desc";
            break;
        case "ip": $order = "contestnumber, sitenumber, logip, logdate desc";
            break;
        default: $order = "contestnumber, sitenumber, logdate desc";
            break;
    }
...
function DBLogInContest($name, $pass, $contest, $msg = true)
{
    ...
    if (!ctype_alnum($name)) {
        ...
        if ($msg) {
            MSGError("Username must be alpha numeric.");
        }
        unset($_SESSION["usertable"]);
        return false;
    }
    ...
    if ($a["userip"] != $gip && $a["userip"] != "" && $a["usertype"] != "score") {
        ...
        if ($msg && $a["usertype"] != "admin" && $a["usermultilogin"] != "t") {
