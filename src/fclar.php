...
CONSTRAINT problem_fk FOREIGN KEY (contestnumber, clarproblem)
    ...
    $r = DBExec(
        $c,
        "CREATE UNIQUE INDEX clar_index ON clartable USING btree ".
           "(contestnumber int4_ops, clarsitenumber int4_ops, clarnumber int4_ops)",
        "DBCreateClarTable(create clar_index)"
    );
    $r = DBExec(
        $c,
        "CREATE INDEX clar_index2 ON clartable USING btree ".
...
function DBUserClars($contest, $site, $user)
{
    ...
    $r = DBExec(
        $c,
        "select distinct c.clarsitenumber as site, c.clarnumber as number, c.clardatediff as timestamp, " .
...
function DBUpdateClarC($contest, $usersite, $usernumber, $clarsite, $clarnumber, $answer, $type, $chief)
{
    ...
    if ($type == "all" && $usersite != $theclar['clarsitenumber']) {
        ...
        DBExec(
            $c,
            "update clartable set clarstatus='$status', clarjudge=$usernumber, clarjudgesite=$usersite, " .
...
function DBClarGiveUp($number, $site, $contest, $usernumber, $usersite)
{
    ...
    DBExec(
        $c,
        "update clartable set clarstatus='openclar', claranswer='', clarjudge=NULL, " .
...
function DBClarDelete($number, $site, $contest, $user, $usersite, $c = null)
{
    ...
    DBExec(
        $c,
        "update clartable set clarstatus='deleted', clarjudge=$user, clarjudgesite=$usersite, updatetime=" .
...
function DBNewClar($param, $c = null, $allowupdate = true)
{
    ...
    if (isset($param['clarsitenumber']) && !isset($param['site'])) {
        $param['site'] = $param['clarsitenumber'];
    }
    ...
    if (isset($param['clarproblem']) && !isset($param['problem'])) {
        $param['problem'] = $param['clarproblem'];
    }
    if (isset($param['clardata']) && !isset($param['question'])) {
        $param['question'] = $param['clardata'];
    }
    ...
    if ($clarnumber < 0) {
        ...
        DBExec(
            $c,
            "update sitetable set sitenextclar=$clarnumber" .
        ...
        if ($n > 0) {
            if (!$allowupdate) {
                if ($cw) {
                    DBExec($c, "rollback work", "DBNewClar(rollback-allowupdate)");
                }
                return false;
            }
            $insert = false;
    ...
    if ($insert) {
        if ($clarinc >= $clarnumber) {
            while (true) {
                ...
                if (DBExecNonStop(
                    $c,
                    "INSERT INTO clartable (contestnumber, clarsitenumber, clarnumber, usernumber, clardate, " .
            ...
            if (!DBExecNonStop(
                ...
                if ($cw) {
                    DBExec($c, "commit work", "DBNewClar(commit-insert-error)");
                }
                return false;
            }
        }
...
function DBGetClarToAnswerC($number, $site, $contest, $chief)
{
    ...
    if ($chief != 1) {
        DBExec(
            $c,
            "update clartable set clarjudge=" . $_SESSION["usertable"]["usernumber"] .
