...
$scoredelay["staff"] = 2;
$actualdelay = 30;
if (isset($scoredelay[$_SESSION["usertable"]["usertype"]])) {
        ...
        for ($rfi = 1;$rfi <= count($rf);$rfi++) {
            ...
            if (isset($lin[1]) && $_SESSION["usertable"]["usertype"] != 'admin') {
                ...
                for ($arri = 0;$arri < count($arr);$arri++) {
                    if ($arr[$arri] != '' && preg_match($arr[$arri], $_SESSION["usertable"]["username"])) {
                        break;
                    }
                }
                if ($arri >= count($arr)) {
            ...
            while (1) {
                $e = key($score);
                $c = current($score);
                if (!isset($score[$e]['classingroup'])) {
                    $score[$e]['classingroup'] = array();
                }
                for ($k = 1;$k < count($grname);$k++) {
                    if ($score[$e]['site'] == $grname[$k]) {
                        $score[$e]['classingroup'][$rfi] = $class;
                        $class++;
                    } elseif (strpos($grname[$k], '/') >= 1) {
                        $u1 = explode('/', $grname[$k]);
            ...
            if ($class > 1) {
                if ($fta) {
                    $fta = false;
                    $strtmp .= "<br><img src=\"$loc/images/smallballoontransp.png\" alt=\"\" onload=\"javascript:toggleGroup($rfi)\"> <b>Available scores:</b> \n";
                }
